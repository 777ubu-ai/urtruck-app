...
test('archived deals never render unread badges and app icon badge uses active Deals attention only', () => {
  ...
  // P0 2026-09-03 (owner-verified fix) — 3-tab канон восстановлен
  // (Предложения/В работе/Архив): вкладка "Архив" исключает unread на
  // строке предложения точно так же, как раньше исключал isClosed.
  // Инвариант не менялся: archive-строка = unread 0.
  assert.match(dealsScreen, /const unread = dealTab === ['"]archive['"] \|\| isClosed[\s\S]{0,100}?\? 0[\s\S]{0,100}?isBidActionable/);
...
test('deal header uses map and status buttons, not the old call button', () => {
  ...
  assert.match(workspace, /compactHeader:\s*\{\s*minHeight:\s*96/);
});
