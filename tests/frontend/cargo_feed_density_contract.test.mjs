...
test('cargo feed keeps the first screen dense enough for narrow mobile browsers', () => {
  ...
  assert.match(cargoFeed, /routeCity:\s*\{[\s\S]*fontSize:\s*16/);
  // DS-2026: 16.5 → 16 (каноническая шкала типографики); плотность карточки
  // сохранена за счёт тех же lineHeight/весов.
  assert.match(cargoFeed, /price:\s*\{[\s\S]*fontSize:\s*16/);
});
...
test('bottom navigation is compact but still keeps the four approved pages', () => {
  ...
  // DS-2026: 10.5 → 11 (канон timestamp/caption); компактность навигации
  // по-прежнему задана PILL_H/LABEL_H выше.
