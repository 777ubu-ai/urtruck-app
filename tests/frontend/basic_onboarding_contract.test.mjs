...
test('basic endpoint validates the minimum profile and sets only basic status', () => {
...
test('register/me and trip publication use an explicit completion gate', () => {
...
test('vehicle success completes onboarding before CreateTrip or Main', () => {
...
test('driver profile continues to vehicle setup without committing role early', () => {
...
test('company and messenger are optional for driver; Pro documents stay separate', () => {
  assert.match(profile, /const validCompany = role === 'driver' \|\|/);
  assert.match(profile, /const validMessenger = role === 'driver' \|\|/);
  for (const route of ['Profile', 'Security', 'Citizenship', 'Identity', 'TruckParams', 'VehicleDocs']) {
