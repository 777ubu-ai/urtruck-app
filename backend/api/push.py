...
def _resolve_ownership(c, table: str, id_col: str, id_val: str,
    ...
    Возвращает (decision, row):
      ...
      "ok"       — можно писать через COALESCE (старое поведение): либо
                   анонимный запрос (никогда не меняет владельца), либо
                   владелец совпадает, либо строка ранее не имела владельца.
      ...
      "conflict" — токен активен, принадлежит ДРУГОМУ пользователю, и
                   device_id не подтверждает то же устройство → 409, ничего
                   не меняем.
    """
    ...
    if requester_user_id is None:
        return "ok", row  # анонимная запись никогда не меняет владельца (COALESCE ниже)
