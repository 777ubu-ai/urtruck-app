...
def _translation_memory_key(source_text: str, source_lang: str | None, target_lang: str):
    from services.translate_service import get_cache_identity

    normalized = unicodedata.normalize("NFC", str(source_text or "")).strip()
    identity = get_cache_identity()
    return (
        hashlib.sha256(normalized.encode("utf-8")).hexdigest(),
        _normalize_lang_code(source_lang) or "auto",
...
def _translation_memory_store(c, source_text: str, source_lang: str | None,
                              target_lang: str, translated_text: str, provider: str):
    key = _translation_memory_key(source_text, source_lang, target_lang)
    # Only store a genuine provider result. The provider/model/prompt identity
    # is part of the primary key, so a later configuration change cannot serve
    # a result generated by a different contract.
    if not translated_text or provider in ("", "stub", "skip", "skip_same_lang"):
        return
    c.execute(
        "INSERT INTO translation_memory "
        "(source_hash,source_lang,target_lang,translated_text,provider,model,prompt_version) "
        "VALUES (?,?,?,?,?,?,?) "
        "ON CONFLICT(source_hash,source_lang,target_lang,provider,model,prompt_version) "
        "DO UPDATE SET translated_text=excluded.translated_text, "
        "last_used_at=CURRENT_TIMESTAMP",
        (*key[:3], translated_text, *key[3:]),
    )
...
def translate_message(body: TranslateIn, user=Depends(require_level(1))):
    ...
    with get_conn() as c:
        ...
        if shared:
            c.execute(
                ...
                "(message_id, target_lang, translated_text, provider) VALUES (?,?,?,?)",
                (body.message_id, target_lang, shared["translated_text"], shared["provider"]),
            )
            return {
                "translated_text": shared["translated_text"],
                "original_text": source_text,
                "target_lang": target_lang,
    ...
    with get_conn() as c:
        ...
        _translation_memory_store(
            c, source_text, source_lang, target_lang,
            result["translated_text"], result["provider"],
        )
    ...
    if target_lang:
        if cached_translation:
            ...
            with get_conn() as c:
                ...
                if shared:
                    translated_text = shared["translated_text"]
                    translation_provider = shared["provider"]
                    translation_cached = True
                    c.execute(
                        "INSERT OR REPLACE INTO chat_translations "
                        "(message_id, target_lang, translated_text, provider) VALUES (?,?,?,?)",
            ...
            # transcription. Only call the provider when neither the
            # per-message nor shared exact-translation cache has a result.
            if not translated_text:
                ...
                try:
                    ...
                    with get_conn() as c:
                        ...
                        _translation_memory_store(
                            c, transcript_text, transcript_lang, target_lang,
                            translated_text, translation_provider,
                        )
                except TranslationError as exc:
