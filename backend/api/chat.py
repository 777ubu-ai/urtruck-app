...
def _deal_key(cargo_id, trip_id, p1: str, p2: str) -> str:
    """2026-07-30 (owner-запрос): один чат на пару пользователей, а не
    на пару+груз/рейс. Раньше ключ учитывал cargo_id/trip_id — тот же
    водитель+клиент, но другой груз, давал ОТДЕЛЬНУЮ комнату; в «Сделках»
    у одного и того же собеседника плодились дубли (часть вообще без
    сообщений). Теперь ключ — только отсортированная пара участников,
    cargo_id/trip_id остаются на комнате как контекст последнего диалога,
    но не участвуют в идентичности комнаты.

    ВАЖНО: не влияет на уже существующие комнаты — _migrate_canonical_rooms
    одноразовая (гейт `if "deal_key" in cols: return`), старые deal_key
    (с префиксом c:/t:) не пересчитываются задним числом. Меняется только
    поведение НОВЫХ комнат, создаваемых с этого момента."""
    return f"p:{p1}:{p2}"
