    ...
    with get_conn() as c:
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS deal_tracking (
                ...
                last_signal_at TEXT,
                updated_at   TEXT DEFAULT CURRENT_TIMESTAMP,
                CHECK (status IN ('pending', 'active', 'declined', 'stopped'))
            )
        """)
        # Existing production SQLite databases already have deal_tracking.
        # Add the evidence/lock fields without rebuilding or dropping a table.
        tracking_cols = {r["name"] for r in c.execute("PRAGMA table_info(deal_tracking)").fetchall()}
        for col, ddl in [
            ("locked_at", "ALTER TABLE deal_tracking ADD COLUMN locked_at TEXT"),
            ("completed_at", "ALTER TABLE deal_tracking ADD COLUMN completed_at TEXT"),
            ("last_signal_at", "ALTER TABLE deal_tracking ADD COLUMN last_signal_at TEXT"),
        ]:
            if col not in tracking_cols:
                c.execute(ddl)
        # State changes are append-only evidence. Coordinates remain in the
        # protected last-point table; this audit trail proves consent and lock.
        c.execute("""
            CREATE TABLE IF NOT EXISTS deal_tracking_events (
...
# Только грузоотправитель подтверждает фактическое получение после
# отметки водителя «Доставлен». Повтор этого статуса тоже не должен
# маскировать попытку водителя завершить сделку от имени клиента.
_SHIPPER_ONLY_STATUSES = {"completed"}
...
def _transition_deal(c, deal: dict, new_status: str, actor_uid: str, request_id: str) -> dict | None:
    ...
    if new_status in _SHIPPER_ONLY_STATUSES:
    ...
    _DEAL_TO_TRIP = {"in_progress": "in_transit", "at_border": "in_transit", "delivered": "delivered", "completed": "delivered", "cancelled": "cancelled"}
...
def _tracking_payload(c, deal_id: str) -> dict:
    row = c.execute(
        "SELECT status, requested_by, requested_at, responded_at, stopped_at, locked_at, completed_at, last_signal_at, updated_at "
...
def _tracking_system_message(deal: dict, text: str) -> None:
    """Best-effort event in the existing deal chat.

    The permission itself is stored server-side in deal_tracking. The message
    is only a visible audit trail for the two participants and must never make
    the consent flow fail if an old database does not yet have chat tables.
    """
    try:
        with get_conn() as c:
...
def get_deal_tracking(deal_id: str, user=Depends(require_level(1))):
    ...
    with get_conn() as c:
        deal = c.execute("SELECT shipper_id, driver_id, status FROM deals WHERE id = ?", (deal_id,)).fetchone()
        if not deal:
            raise HTTPException(status_code=404, detail="Сделка не найдена")
        if user["id"] not in (deal["shipper_id"], deal["driver_id"]):
            raise HTTPException(status_code=403, detail="Нет доступа к сделке")
        payload = _tracking_payload(c, deal_id)
    return {"ok": True, "tracking": payload}


@mp_router.get("/tracking/active")
def list_active_driver_tracking(user=Depends(require_level(1))):
    """The driver's app polls this small endpoint to decide which background
    task IDs may transmit. It deliberately returns *only* server-approved,
    currently active deals — never a client-side guess based on deal status."""
    with get_conn() as c:
        rows = c.execute(
            "SELECT dt.deal_id FROM deal_tracking dt JOIN deals d ON d.id = dt.deal_id "
            "WHERE d.driver_id = ? AND dt.status = 'active' "
            "AND d.status IN ('in_progress','at_border')",
            (user["id"],),
        ).fetchall()
    return {"ok": True, "deal_ids": [r["deal_id"] for r in rows]}
...
def request_deal_tracking(deal_id: str, user=Depends(require_level(1))):
    ...
    with get_conn() as c:
        ...
        if deal["shipper_id"] != user["id"]:
            raise HTTPException(status_code=403, detail="Запросить GPS может только грузоотправитель")
        if deal["status"] not in _TRACKING_DEAL_STATUSES:
            raise HTTPException(status_code=409, detail="Отслеживание доступно только для активной сделки")
        current = _tracking_payload(c, deal_id)
        if current.get("locked_at"):
            return {"ok": True, "tracking": current, "already_locked": True}
        if current.get("status") in ("pending", "active"):
            if current.get("status") == "active" and deal["status"] in ("in_progress", "at_border"):
                c.execute(
                    "UPDATE deal_tracking SET locked_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE deal_id=?",
                    (deal_id,),
                )
                current = _tracking_payload(c, deal_id)
            return {"ok": True, "tracking": current, "already_pending": current.get("status") == "pending"}
        c.execute(
            "INSERT INTO deal_tracking (deal_id, status, requested_by, requested_at, responded_at, stopped_at, updated_at) "
            "VALUES (?, 'pending', ?, CURRENT_TIMESTAMP, NULL, NULL, CURRENT_TIMESTAMP) "
            "ON CONFLICT(deal_id) DO UPDATE SET status='pending', requested_by=excluded.requested_by, "
            "requested_at=CURRENT_TIMESTAMP, responded_at=NULL, stopped_at=NULL, updated_at=CURRENT_TIMESTAMP",
            (deal_id, user["id"]),
        )
...
@mp_router.post("/deals/{deal_id}/tracking/respond")
def respond_deal_tracking(deal_id: str, body: TrackingDecisionIn, user=Depends(require_level(1))):
    """Driver explicitly allows or declines GPS after the operating system has
    granted location permission. No permission request means no active state."""
    decision = (body.decision or "").strip().lower()
    if decision not in ("approve", "decline"):
        raise HTTPException(status_code=400, detail="decision должен быть approve или decline")
    with get_conn() as c:
        row = c.execute("SELECT * FROM deals WHERE id = ?", (deal_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Сделка не найдена")
        ...
        if deal["driver_id"] != user["id"]:
            raise HTTPException(status_code=403, detail="Подтвердить GPS может только водитель")
        if deal["status"] not in _TRACKING_DEAL_STATUSES:
            raise HTTPException(status_code=409, detail="Отслеживание доступно только для активной сделки")
        current = _tracking_payload(c, deal_id)
        if current.get("status") != "pending":
            raise HTTPException(status_code=409, detail="Нет ожидающего запроса GPS")
        new_status = "active" if decision == "approve" else "declined"
        lock_sql = ", locked_at=COALESCE(locked_at, CURRENT_TIMESTAMP)" if (
            new_status == "active" and deal["status"] in ("in_progress", "at_border")
        ) else ""
        c.execute(
            "UPDATE deal_tracking SET status = ?, responded_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP" + lock_sql + " "
            "WHERE deal_id = ?",
            (new_status, deal_id),
        )
...
@mp_router.post("/deals/{deal_id}/tracking/stop")
def stop_deal_tracking(deal_id: str, user=Depends(require_level(1))):
    """The driver may withdraw voluntary consent only before pickup. Once the
    shipment is in transit, tracking is locked to the deal as evidence."""
    with get_conn() as c:
        row = c.execute("SELECT * FROM deals WHERE id = ?", (deal_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Сделка не найдена")
