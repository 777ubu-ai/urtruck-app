...
- `src/utils/i18n.js` + `useI18n.js` — 16 языков (I18N-16, 2026-09-12): исходные RU/EN/ZH/KK (1963 ключа на 2026-09-08, полная символичная симметрия — `Object.keys()` на реально загруженном модуле, ноль расхождений; проверяется `node qa/utils/i18nSmoke.js`, уже включён в `npm run qa:i18n`) + добавленные UZ/KY/TG/DE/FR/PL/LT/LV/IT/TR/BE/RO с осознанно ограниченным набором CORE_KEYS (277 ключей на 2026-09-12: auth/registration/statuses/cargo/gates/validation/photo-voice/errors/navigation/filters/chat-attachments/GPS-map/push, плюс 31 backend-error-код) — ключи вне CORE_KEYS у новых локалей отсутствуют и **осознанно** резолвятся через существующий EN-фолбэк в `t()`/`translate()` (никогда не RU). `src/utils/localeRegistry.js` — единый канонический реестр локалей (code/nativeName/englishName/transliteration/rtl/enabled/fallback, item 8 verbatim native labels), single source of truth для будущего UI-селектора языка (Kimi/Codex) — не дублировать список локалей по экранам. Полнота проверяется `node qa/utils/localeCompleteness.js`. Все пользовательские тексты обязаны идти через `t(...)`. ⚠️ *7 живых экранов дополнительно держат собственные локальные `COPY`-словари в обход этого файла — не баг, но известный архитектурный долг, см. `docs/CURRENT_ENGINEERING_CANON.md`, п.5.*
