...
- `src/utils/i18n.js` + `useI18n.js` — engineering-слой поддерживает 16 языков (I18N-16, ветка `feat/claude-i18n-16-locales-20260912`, PR #357+split): исходные RU/EN/ZH/KK (1963 ключа на 2026-09-08, полная симметрия — `Object.keys()` на реально загруженном модуле, ноль расхождений) + добавленные UZ/KY/TG/DE/FR/PL/LT/LV/IT/TR/BE/RO. ⚠️ *Это готовность контента, НЕ решение о запуске: список из 16 локалей — инженерный артефакт, "product-approved locales" (какие языки реально включать в проде и когда) остаётся за владельцем продукта и требует явного product approval — как и связанное изменение push-фолбэка RU→EN (`backend/services/push_i18n.py` DEFAULT_LOCALE), которое отдельным PR ждёт того же approval и до него не должно менять поведение для существующих пользователей без сохранённой locale.* Реальное покрытие — из полного скана `t()`-вызовов в `src/` (912 продакшен-ключей на 2026-09-13, не 277 — прежняя оценка CORE_KEYS устарела и заменена полным аудитом), 907/912 переведено на каждую из 12 новых локалей; 5 ключей документированно language-neutral (см. `EXCLUDE_NEUTRAL` в `qa/utils/localeCompleteness.js`) — это не пропуски. Ключ вне переведённого набора резолвится через EN-фолбэк в `t()`/`translate()` (никогда не RU) — фолбэк-политика не менялась. Проверяется `node qa/utils/i18nSmoke.js` (`npm run qa:i18n`) и `node qa/utils/localeCompleteness.js` (per-locale translated/fallback/missing). `src/utils/localeRegistry.js` — единый канонический реестр локалей (code/nativeName/englishName/transliteration/rtl/enabled/fallback, item 8 verbatim native labels), single source of truth для будущего UI-селектора языка (Kimi/Codex) — не дублировать список локалей по экранам. Все пользовательские тексты обязаны идти через `t(...)`. ⚠️ *7 живых экранов дополнительно держат собственные локальные `COPY`-словари в обход этого файла — не баг, но известный архитектурный долг, см. `docs/CURRENT_ENGINEERING_CANON.md`, п.5.*
