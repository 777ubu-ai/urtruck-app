...
- `Deals` (`NotificationsScreen`) — вкладка **«Сделки»**, иконка **рукопожатие**
  (`handshake` из MaterialCommunityIcons — в Feather рукопожатия нет). Единый
  инбокс живой работы: ставки, торг, статусы сделок, уведомления. Это бывший
  колокольчик, спущенный вниз. Бейдж непрочитанного `notifUnread` — на `Deals`
  (не на `MyWork`). Заголовок экрана тоже «Сделки» (t('tab_deals')).
- `Chat` у **водителя** нельзя прятать — отдельная вкладка.
- `Queue` — отдельная обычная центральная вкладка (driver).
- **Client tab-bar = 3 вкладки** (приказ владельца 2026-07-26): `MyWork`
  («Грузы») / `Feed` («Машины») / `Deals` («Сделки»). У клиента:
  - вкладок «Разместить» (центральный «+») и «Чаты» НЕТ. Размещение груза —
    кнопка «Разместить груз» ВНУТРИ «Мои грузы» (как у водителя §2.2.2).
  - `Deals` у клиента = `ChatsListScreen` в dealsMode (route.name==='Deals'):
    сверху «Предложения (N)» — входящие ставки, тап → комната сделки; ниже —
    все переписки. Торг (BargainCard) и чат живут ВНУТРИ комнаты (`ChatScreen`).
  - в «Мои грузы» у клиента НЕТ под-вкладки «Предложения» — только
    Мои грузы / Везут / Доставлено (+ Архив). Плашка «N предложений» на
    карточке груза ведёт во вкладку «Сделки».
  - колокольчик в шапке НЕ возвращать (пушей достаточно — прямое указание).
  - водителя эта перестройка пока НЕ касается; переход водителя на ту же
    схему — только после отдельной команды владельца.
- Кнопка `Publish`/«Разместить» не должна становиться вкладкой ни у одной роли.
