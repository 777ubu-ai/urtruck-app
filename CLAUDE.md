...
- Цвета (редизайн 08.08.2026, единая светлая зелёная B2B-тема): bg `#F6F8F7`, card `#FFFFFF`, border `#E5ECE8`, text `#14221C`, muted `#617067`; бренд-зелёный водителя `#168759` (WCAG: 4.52:1 с белым текстом), deep `#0F6B47`, тинт `#E8F6EF`; **фирменный оранжевый грузоотправителя (решение владельца 28.08.2026)** — токен `clientBrand`: LIGHT `#C25700` (4.51:1 с белым, AA), DARK `#FF8400` (6.94:1 на тёмном), подложка `clientBrandFill`; яркий `#FF8400`/`#F59E0B` в LIGHT — ТОЛЬКО декоративные фоны/плашки цены-ожидания-предупреждений (как текст/иконка на белом даёт 2.46:1 — запрещён), как ТЕКСТ на белом — `#E06D00`; error `#D64545`/`#EF4444`; info `#3478D4`; рейтинг `#D97706` (янтарь, 3.19:1); placeholder `#6B7A71`.
- Текст поверх зелёных кнопок (`#168759`) — белый `#FFFFFF` (4.52:1, AA). Крупные CTA — на deep `#0F6B47` (6.5:1). Источник истины по зелёному: `v1AccentFor(role)` в `theme/designV1.js` (обе роли дают один зелёный). Роль-акцент грузоотправителя брать ТОЛЬКО из `v1.clientBrand` (`useV1Colors()`), никогда хардкодом: прежний `isDriver ? '#168759' : '#FF8400'` в `MyTripsScreen`/`ProfileScreen` делал кнопку «Принять» и иконки клиента нечитаемыми (2.46:1). Заперто тестом `tests/frontend/test_client_brand_accent.mjs`.
...
- **Client tab-bar = 4 вкладки** (уточнено 28.08.2026 по решению владельца —
  раньше здесь было «3 вкладки», код всегда давал 4 и это верно: граница
  важна и грузоотправителю): `MyWork` («Грузы») / `Feed` («Машины») /
  `Deals` («Сделки») / `Queue` («Очередь»).
  Запрет на 4 вкладки снят; источник истины — `MainTabs` в `AppNavigator.js`,
  контракт заперт в `tests/frontend/test_navigation_tabs.mjs`.
