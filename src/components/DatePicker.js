...
export default function DatePicker({ value, onChange, placeholder = 'DD.MM.YYYY', style, min, max, defaultOpen = false }) {
  ...
  const { t } = useI18n();
  // PR-A (P0-1 DatePicker дубль): когда DatePicker встраивается под Field-обёрткой
  // в форму (см. CreateCargoScreen), у внешнего Field уже есть label+value preview.
  // Чтобы не было дубля «Дата сверху + 📅 ДД.ММ.ГГГГ снизу», новый prop
  // `defaultOpen` инициализирует календарь сразу открытым И отключает
  // собственный preview-row.
  ...
  return (
    <View style={[s.wrapper, style]}>
      {/* P1-7: пока календарь открыт, не показываем preview-полоску — иначе
          ...
          PR-A (P0-1): когда `defaultOpen=true` (форма уже имеет Field-row
          триггера сверху), preview мы вообще не рендерим, иначе получается
          двойной date row. */}
