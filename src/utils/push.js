...
export const push = {
  ...
  async registerNative() {
    ...
    try {
      ...
      appId = Constants?.expoConfig?.android?.package ||
        Constants?.expoConfig?.ios?.bundleIdentifier ||
        Constants?.manifest?.android?.package ||
        Constants?.manifest?.ios?.bundleIdentifier || null;
    ...
    try {
      ...
      if (!nativeTokenData?.data) return { ok: false, reason: 'token_failed', error: String(e) };
    }
    ...
    const registerToken = async ({ pushToken, provider }) => {
      try {
        ...
        let userId;
        try { userId = (await resp.json())?.user_id; } catch {}
        dbg('register-native', provider, '→', resp.status, 'user_id=', userId);
        if (resp.status === 409) return { ok: false, reason: 'token_conflict', token: pushToken, status: resp.status, provider };
        if (resp.status < 200 || resp.status >= 300) return { ok: false, reason: 'register_rejected', token: pushToken, status: resp.status, provider };
        if (authToken && !userId) return { ok: false, reason: 'not_linked', token: pushToken, status: resp.status, provider };
        return { ok: true, token: pushToken, user_id: userId, provider };
      } catch (e) {
    ...
    if (nativeTokenData?.data) {
      registrations.push(await registerToken({
        pushToken: nativeTokenData.data,
        provider: Platform.OS === 'android' ? 'fcm' : 'apns',
      }));
    }
    const failed = registrations.find((r) => !r?.ok);
    if (failed?.reason === 'token_conflict') {
