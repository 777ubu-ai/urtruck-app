...
// is RU-only prose (backend/api/verification_gate.py's LEVEL_REQUIREMENTS),
// but the same response carries a stable structured `required_name` (guest/
// phone_verified/identity_verified/driver_verified). Translate that via
// verification_hint_<required_name> (all 16 locales) BEFORE falling through
// to the raw RU hint below — identical fix already applied in marketAPI.js;
// registration.js was missed in that pass (PR #357 review finding).
...
// not part of the public regAPI surface used by screens.
export const __testables = { normalizeDetail, localizedVerificationHint };
