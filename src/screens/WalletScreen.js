  ...
  return (
    <SafeAreaView style={[s.container, { backgroundColor: v1.bg }]} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <GradientText style={s.title} colors={['#168A5B', '#16A34A']}>{t('wallet')}</GradientText>
        ...
        <View style={[s.balanceCard, { backgroundColor: theme.card, borderColor: accent + '30' }]}>
          <View style={[s.betaBadge, { flexDirection: 'row', alignItems: 'center', gap: 6 }]}>
            <Feather name="gift" size={14} color="#168A5B" />
          ...
          <View style={[s.section, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            {transactions.map((tx, i) => (
              <View key={tx.id} style={[s.txRow, i < transactions.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
                <View style={[s.txIcon, { backgroundColor: tx.amount > 0 ? '#168A5B20' : '#EF444420' }]}>
                ...
                <Text style={[s.txAmount, { color: tx.amount > 0 ? '#168A5B' : '#EF4444' }]}>
...
const s = StyleSheet.create({
  ...
  betaBadge: { backgroundColor: '#168A5B15', paddingHorizontal: 14, paddingVertical: 5, borderRadius: 12, marginBottom: 10 },
  betaBadgeText: { color: '#168A5B', fontSize: 12, fontWeight: '600' },
