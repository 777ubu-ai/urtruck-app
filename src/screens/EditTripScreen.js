        ...
        <View style={[s.row, { marginBottom: 10 }]} onLayout={(e) => { priceInputY.current = e.nativeEvent.layout.y; }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }} style={{ flex: 3 }}>
            ...
            {['KZT', 'USD', 'RUB', 'CNY'].map(k => (
              <TouchableOpacity
                key={k}
                style={[s.currChip, { backgroundColor: theme.card, borderColor: theme.border }, currency === k && { backgroundColor: '#168A5B', borderColor: '#168A5B' }]}
...
const s = StyleSheet.create({
  ...
  previewPrice: { color: '#168A5B', fontSize: 26, fontWeight: '900' },
  saveBtn: { backgroundColor: '#168A5B', borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 18 },
