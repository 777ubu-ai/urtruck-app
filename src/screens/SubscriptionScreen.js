...
export default function SubscriptionScreen({ navigation, route }) {
  ...
  // Отмена Google Play-подписки возможна только в интерфейсе Google Play
  // (политика магазина) — открываем нативный экран управления подпиской,
  // актуальный статус вернётся на сервер через RTDN-webhook.
  ...
  const manageSubscription = useCallback(async () => {
    ...
    try {
      await RNIap.deepLinkToSubscriptions({ sku: status?.google_product_id });
    } catch {
      try {
        await Linking.openURL(PLAY_SUBSCRIPTIONS_URL);
      } catch {
        toast(t('subscription_manage_error'));
      }
    }
  }, [status?.google_product_id, toast, t]);
      ...
      {loading ? (
        ...
        <View style={s.body}>
          <View style={[s.card, { backgroundColor: v1.surface, borderColor: v1.border }]}>
            ...
            {isActive && status?.auto_renewing != null && (
              <Text style={[s.cardSub, { color: v1.textMuted }]}>
                {status.auto_renewing ? t('subscription_autorenew_on') : t('subscription_autorenew_off')}
              </Text>
            )}
            {isActive && !!status?.period_end && (
              <Text style={[s.cardSub, { color: v1.textMuted }]}>
                {(t('subscription_valid_until') || '').replace('{date}', formatPeriodEnd(status.period_end))}
              </Text>
            )}
          </View>

          {isActive && !isPilotFree && (Platform.OS === 'web' ? (
            <TouchableOpacity onPress={manageSubscription} testID="subscription-manage-button">
              <Text style={[s.manageLink, { color: accent }]}>{t('subscription_manage_cta')}</Text>
            </TouchableOpacity>
          ) : (
            <View style={s.manageBlock}>
              <TouchableOpacity
                style={[s.manageBtn, { borderColor: accent }]}
                onPress={manageSubscription}
                testID="subscription-manage-button"
              >
                <Text style={[s.manageBtnText, { color: accent }]}>{t('subscription_manage_cta')}</Text>
              </TouchableOpacity>
              <Text style={[s.manageHint, { color: v1.textMuted }]}>{t('subscription_cancel_hint')}</Text>
            </View>
          ))}
...
const s = StyleSheet.create({
  ...
  manageBlock: { gap: 8 },
  manageBtn: { borderRadius: 14, borderWidth: 1, paddingVertical: 14, alignItems: 'center', justifyContent: 'center' },
  manageBtnText: { fontSize: 16, fontWeight: '600' },
  manageHint: { fontSize: 12, textAlign: 'center' },
  manageLink: { fontSize: 14, fontWeight: '600', textAlign: 'center', textDecorationLine: 'underline' },
});
