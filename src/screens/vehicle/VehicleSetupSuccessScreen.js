...
export default function VehicleSetupSuccessScreen({ navigation, route }) {
  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
    ...
    <ScrollView contentContainerStyle={[styles.scroll, { alignItems: 'center' }]}>
      ...
      {basicState === 'error' ? <><Text style={styles.error}>{c.saveErrorSub}</Text><Pressable testID="basic-onboarding-retry" onPress={completeBasic} style={styles.secondary}><Text style={styles.secondaryText}>{c.retry}</Text></Pressable></> : null}
      <Pressable disabled={basicState !== 'done'} onPress={() => { if (basicState === 'done') navigation.replace('Main', { role: 'driver' }); }} style={[{ width: '100%', marginTop: 10 }, basicState !== 'done' && { opacity: 0.5 }]}><View style={styles.secondary}><Text style={styles.secondaryText}>{c.home}</Text></View></Pressable>
