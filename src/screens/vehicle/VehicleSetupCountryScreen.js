...
import { brand } from '../../theme/brandV2';
...
export default function VehicleSetupCountryScreen({ navigation, route }) {
  ...
  const next = async () => { if (!draft.driver_citizenship_country_code || !draft.vehicle_registration_country_code) { setError(c.invalid); return; } setError(''); const saved = await regAPI.saveDriverDraft({ driver_citizenship_country_code: draft.driver_citizenship_country_code, vehicle_registration_country_code: draft.vehicle_registration_country_code }); if (!saved.ok) { setError(saved.detail || c.saveError); return; } navigation.navigate('VehicleSetupMachine', { ...route?.params }); };
  if (loading) return <SafeAreaView style={styles.safe}><ActivityIndicator color={brand.primary} style={{ marginTop: 80 }} /></SafeAreaView>;
