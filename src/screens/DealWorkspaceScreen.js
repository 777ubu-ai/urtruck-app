...
const COPY = {
  RU: {
    ...
    routeUnavailable: "Дорожный расчёт временно недоступен",
    attachPhoto: "Фото",
    attachCamera: "Камера",
  ...
  EN: {
    ...
    routeUnavailable: "Road route estimate is temporarily unavailable",
    attachPhoto: "Photo",
    attachCamera: "Camera",
  ...
  KK: {
    ...
    routeUnavailable: "Жол маршрутын есептеу уақытша қолжетімсіз",
    attachPhoto: "Фото",
    attachCamera: "Камера",
...
export default function DealWorkspaceScreen({ navigation, route }) {
  // Legacy route shim: old deep links and cached web bundles must render the
  // same canonical workspace as driver/shipper now.
  return <DealWorkspaceRoute navigation={navigation} route={route} />;
  ...
  const panResponder = React.useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderGrant: () => {
          dragStart.current = heightForState(sheetState);
        },
        onPanResponderMove: (_, gesture) => {
          const next = Math.max(
            collapsedHeight,
            Math.min(fullHeight, dragStart.current - gesture.dy),
          );
  ...
  React.useEffect(() => {
    ...
    const resolve = async () => {
      try {
        ...
        /* fail closed to the data already supplied by navigation */
      }
  ...
  const refreshDeal = React.useCallback(async () => {
    ...
    try {
      ...
      /* screen keeps last authoritative response */
    }
  }, [dealId]);
  ...
  const loadMessages = React.useCallback(async () => {
    ...
    try {
      ...
      /* keep visible messages on a transient poll error */
    }
  ...
  const trackingActive = Boolean(
    dealId && TRACKING_STATUSES.includes(deal?.status),
  );
  const refreshLocation = React.useCallback(async () => {
    ...
    try {
      ...
      if (mounted.current && result?.has_location && result.location)
  ...
  const nextAction = React.useMemo(() => {
    ...
    if (isDriver) {
      ...
      if (deal.status === "in_progress" && deal.is_international === true)
        return {
          key: "at_border",
          label: t("mark_at_border"),
          icon: "map-pin",
        };
      if (deal.status === "in_progress" && deal.is_international == null)
        return {
          key: "clarify",
      ...
      if (deal.status === "in_progress" || deal.status === "at_border")
        return { key: "delivered", label: t("mark_arrived"), icon: "package" };
    }
    if (isShipper && deal.status === "delivered")
      return {
        key: "completed",
          ...
          >
            {item.photo && item.mediaUrl ? (
              <TouchableOpacity
                activeOpacity={0.82}
                onPress={() =>
                  setImagePreview({ uri: item.mediaUrl, time: item.time })
                }
  ...
  const statusLabel = formatStatus(
    userFacingDealStatus(deal?.status || "accepted"),
  );
      ...
      >
        ...
        >
          ...
          <View style={s.headerText}>
            ...
            {meta ? (
              <Text
                style={[s.metaText, { color: colors.textMuted }]}
                numberOfLines={1}
              >
                {meta}
              </Text>
        ...
        <View style={s.mapArea} testID="deal-map-first-area">
          ...
          {nextAction ? (
            <TouchableOpacity
              style={[
                s.floatingAction,
                {
                  backgroundColor: nextAction.disabled ? "#E4E8E5" : "#168759",
                },
              ]}
          ...
          <View
            style={[
              s.mapQuickRow,
              { bottom: collapsedHeight + (routeSummary ? 88 : 16) },
            ]}
          >
            <TouchableOpacity
              style={[
              ...
              onPress={() => showTab("docs")}
              testID="deal-documents-chip"
            >
              <Feather name="file-text" size={15} color="#168759" />
              <Text style={[s.mapQuickText, { color: colors.text }]}>
                {ui.documents}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              onPress={() => showTab("status")}
              testID="deal-statuses-chip"
            >
          ...
          {routeSummary ? (
            ...
            >
              ...
              <View
                style={[s.metricDivider, { backgroundColor: colors.border }]}
              />
              <View style={s.metricCell}>
                <Text style={[s.metricLabel, { color: colors.textMuted }]}>
        ...
        <Animated.View
          ...
          testID={`deal-chat-sheet-${sheetState}`}
        >
          <View
            {...panResponder.panHandlers}
            style={s.dragZone}
            testID="deal-chat-drag-handle"
          >
          ...
          <View style={[s.sheetHeader, { borderBottomColor: colors.border }]}>
            ...
            >
              ...
              <View style={s.sheetTitleText}>
                <View style={s.sheetTitleRow}>
                  ...
                  {sheetTab === "chat" && unreadCount > 0 ? (
                    <Text style={s.newCount}>
                      {unreadCount} {ui.newMessages}
                    </Text>
                  ) : null}
                </View>
            ...
            {sheetState !== "collapsed" ? (
              <TouchableOpacity
                onPress={() => setSheet("collapsed")}
                style={s.collapseButton}
                testID="deal-chat-collapse"
              >
          ...
          {sheetState !== "collapsed" ? (
            <>
              <View style={s.tabRow}>
                {[
                  ["chat", ui.messages, "message-circle"],
                  ["docs", ui.documents, "file-text"],
                  ["status", ui.statuses, "activity"],
                ].map(([key, label, icon]) => (
                  <TouchableOpacity
                    key={key}
                    style={[s.tab, sheetTab === key && s.tabActive]}
                    onPress={() => setSheetTab(key)}
                    testID={`deal-sheet-tab-${key}`}
                  >
                    <Feather
                      name={icon}
                      size={14}
                      color={sheetTab === key ? "#168759" : colors.textMuted}
                    />
                    <Text
                      style={[
                        s.tabText,
                        {
                          color:
                            sheetTab === key ? "#168759" : colors.textMuted,
                        },
                      ]}
                      numberOfLines={1}
                    >
                    ...
                    >
                      ...
                      >
                        <View style={s.attachIcon}>
                          <Feather name="image" size={20} color="#168759" />
                        </View>
                        <Text style={[s.attachLabel, { color: colors.text }]}>
                          {ui.attachPhoto}
                        </Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                      ...
                      >
                        <View style={s.attachIcon}>
                          <Feather name="camera" size={20} color="#168759" />
                        </View>
                        <Text style={[s.attachLabel, { color: colors.text }]}>
                          {ui.attachCamera}
                        </Text>
                      ...
                      >
                        <View style={s.attachIcon}>
                          <Feather name="file-text" size={20} color="#168759" />
                        </View>
                        <Text style={[s.attachLabel, { color: colors.text }]}>
                          {ui.attachDocument}
                        </Text>
                        ...
                        setInputHeight(
                          Math.max(
                            ...
                            Math.min(
                              108,
                              Math.ceil(
                                event.nativeEvent.contentSize.height + 18,
                              ),
                            ),
                          ),
                        )
                ...
                <View style={s.panelBody} testID="deal-status-panel">
                  <FlatList
                    ...
                    keyExtractor={(item, index) =>
                      String(item.id || item.event_type || index)
                    }
