...
export default function DealWorkspaceScreenV2({ navigation, route }) {
  ...
  React.useEffect(() => {
    ...
    const timer = setInterval(
      () =>
        setRecordSecs(
          Math.max(0, Math.floor((Date.now() - recordStartRef.current) / 1000)),
        ),
      500,
    );
  ...
  const refreshDeal = React.useCallback(async () => {
    ...
    try {
      ...
      if (server.counterparty_name)
        setPartner((prev) => ({
          ...(prev || {}),
          name: server.counterparty_name,
        }));

      const detail = {};
      if (server.cargo_id) {
        try {
          detail.cargo = await marketAPI.getCargo(server.cargo_id);
        } catch {}
      }
      if (server.trip_id) {
        try {
          detail.trip = await marketAPI.getTrip(server.trip_id);
        } catch {}
      }
  ...
  const refreshTimeline = React.useCallback(async () => {
    ...
    try {
      ...
      if (mounted.current)
        setTimeline(Array.isArray(result?.events) ? result.events : []);
    } catch {
      /* no fake timeline */
    }
  }, [dealId]);
  ...
  const toggleVoiceTranscript = React.useCallback(
    async (item) => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      for (const item of pending.slice(0, 6)) {
        try {
          const result = await chatAPI.translate(
            item.id,
            getLanguage().toLowerCase(),
          );
          ...
          /* translation is an assistive layer, never blocks chat */
        }
      }
    })();
    return () => {
  ...
  const trackingActive = Boolean(
    dealId && LIVE_TRACKING_STATUSES.includes(deal?.status),
  );
  ...
  const routePoints = React.useMemo(
    () => dedupePoints([...parseRouteCities(from), ...parseRouteCities(to)]),
    [from, to],
  );
  ...
  const onRouteSummary = React.useCallback(
    (summary) => setRouteSummary(summary || null),
    [],
  );
  ...
  const updatedText = React.useMemo(() => {
    ...
    if (minutes < 1440)
      return `${ui.updated} ${Math.floor(minutes / 60)} ${ui.hour} ${ui.ago}`;
    return `${ui.updated} ${Math.floor(minutes / 1440)} ${ui.day} ${ui.ago}`;
  }, [location?.updated_at, ui]);

  const changeDealStatus = React.useCallback(
    async (nextStatus) => {
  ...
  const retryFailedText = React.useCallback(
    (item) => {
  ...
  const appendOptimisticVoice = React.useCallback(
    (uri, duration) => {
      const clientId = newClientId();
      setMessages((items) => [
        ...items,
  ...
  const toggleAutoTranslate = React.useCallback(() => {
    ...
    setAutoTranslate((current) => {
      const next = !current;
      toast(
        next ? t("autotranslate_on") : t("autotranslate_off"),
        "info",
        1800,
      );
      return next;
  ...
  const sendPhoto = React.useCallback(
    async (camera) => {
      try {
        ...
        let uri = source;
        try {
          uri = await compressImage(source, { maxSide: 1200, quality: 0.75 });
        } catch {}
        const clientId = newClientId();
        setMessages((items) => [
          ...items,
        ...
        toast(
          error?.isNetwork ? t("no_connection") : t("chat_send_failed"),
          "error",
        );
      }
    },
  ...
  const uploadDocument = React.useCallback(
    async (docItem) => {
      setMessages((items) =>
        items.map((m) =>
          m.id === docItem.id
            ? { ...m, docStatus: "uploading", docErrorText: null }
            : m,
        ),
      );
      try {
        ...
        setMessages((items) =>
          items.map((m) =>
            m.id === docItem.id
              ? { ...m, docStatus: "failed", docErrorText: t(key) }
              : m,
          ),
        );
      }
    ...
    ({ item }) => {
      if (item.system)
        return (
          <View style={s.systemRow}>
            <Text style={[s.systemText, { color: colors.textMuted }]}>
              {item.text}
            </Text>
          </View>
        );
      ...
      if (item.kind === "document") {
        ...
        const isBusy =
          ...
          item.docStatus === "queued" ||
          item.docStatus === "retrying";
        const isFailed = item.docStatus === "failed";
        return (
          <View
            style={[s.messageRow, item.mine ? s.messageMine : s.messageThem]}
          >
            <TouchableOpacity
              activeOpacity={item.docUrl ? 0.72 : 1}
              disabled={!item.docUrl}
              onPress={() =>
                item.docUrl && Linking.openURL(item.docUrl).catch(() => {})
              }
              style={[
                s.docBubble,
              ...
              <View style={s.docMainRow}>
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  <Text
                    ...
                    style={[
                      s.docName,
                      { color: item.mine ? "#111B21" : colors.text },
                    ]}
                  >
                    {item.docName}
                  </Text>
                  ...
                  >
                    {isFailed
                      ...
                      : [
                          formatBytes(item.docSize),
                          isBusy
                            ? t("chat_attach_status_uploading")
                            : t("chat_attach_status_uploaded"),
                        ]
                          .filter(Boolean)
                          .join(" · ")}
                  </Text>
                ...
                {isFailed ? (
                  <TouchableOpacity
                    onPress={() => retryDocument(item)}
                    style={s.docRetryBtn}
                    testID="deal-chat-document-retry"
                  >
                    <Feather
                      name="refresh-cw"
                      size={15}
                  ...
                  <Feather
                    name="chevron-right"
                    size={16}
                    color={item.mine ? "rgba(17,27,33,0.58)" : colors.textMuted}
                  />
                ) : null}
              </View>
        ...
        <View style={[s.messageRow, item.mine ? s.messageMine : s.messageThem]}>
          ...
          >
            {item.photo && item.mediaUrl ? (
              <TouchableOpacity
                onPress={() => setFullImage(item.mediaUrl)}
                testID="deal-chat-photo-bubble"
              >
                <View style={s.photoShell}>
                  ...
                  <View style={s.photoMetaOverlay}>
                    <Text
                      style={[
                        s.messageTime,
                        { color: "rgba(255,255,255,0.92)", marginTop: 0 },
                      ]}
                    >
                      {item.time}
                    </Text>
              ...
              <>
                ...
                >
                  ...
                  >
                    {ui.voiceMessage}
                    {item.voiceDuration
                      ? ` · ${item.voiceDuration}${t("unit_sec_short")}`
                      : ""}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                ...
                >
                  {voiceTranscribing !== item.id ? (
                    <Feather
                      ...
                      size={11}
                      color={
                        item.mine ? "rgba(17,27,33,0.58)" : colors.textMuted
                      }
                    />
                  ) : null}
                  <Text
                  ...
                  >
                    {voiceTranscribing === item.id
                      ? "..."
                      : voiceTranscripts[item.id]?.visible
                        ? t("voice_hide_text")
                        : voiceTranscripts[item.id]?.transcriptText
                          ? t("voice_show_text")
                          : t("voice_to_text")}
                  </Text>
                </TouchableOpacity>
                ...
                voiceTranscripts[item.id]?.transcriptText ? (
                  <View
                    style={[
                      s.transcriptBlock,
                      item.mine ? s.transcriptBlockMine : s.transcriptBlockThem,
                    ]}
                  >
                    <Text
                      style={[
                    ...
                    <Text
                      style={[
                        s.transcriptText,
                        { color: item.mine ? "#FFFFFF" : colors.text },
                      ]}
                    >
                      {voiceTranscripts[item.id].transcriptText}
                    </Text>
                    {voiceTranscripts[item.id].translatedText ? (
                      <>
                        <Text
                          style={[
                            s.transcriptLabel,
                ...
                />
                <View style={s.msgFooter}>
                  <Text style={[s.messageTime, { color: item.mine ? "rgba(17,27,33,0.58)" : colors.textMuted }]}>{item.time}</Text>
                </View>
              </>
            ) : item.text ? (
              <>
                <View style={s.msgBodyRow}>
                  ...
                  >
                    ...
                    !translations[item.id].showOriginal
                ...
                {!item.mine && !item.system ? (
                  ...
                  >
                    <Feather
                      ...
                      size={11}
                      color={
                        item.mine ? "rgba(255,255,255,0.65)" : colors.textMuted
                      }
                    />
                    <Text
                      style={[
          ...
          {item.sendStatus === "failed" && !item.voice ? (
            <TouchableOpacity
              ...
              style={s.errorRow}
              testID="deal-chat-message-retry"
            >
              <Feather name="alert-circle" size={12} color="#EF4444" />
              <Text style={s.errorText} numberOfLines={2}>
                {item.sendError || t("chat_send_failed")} ·{" "}
                {t("chat_attach_retry")}
              </Text>
            </TouchableOpacity>
          ) : null}
  ...
  const latestPreview = latestMessage
    ? latestMessage.kind === "document"
      ? `📎 ${latestMessage.docName}`
      : latestMessage.text ||
        (latestMessage.voice
          ? ui.voiceMessage
          : latestMessage.photo
  ...
  const cargoMeta = [
    ...
    deal?.amount != null
      ? formatPrice(
          deal.amount,
          deal.currency || cargo?.currency || trip?.currency || "USD",
          t,
        )
      : null,
  ]
  ...
  const counterpartyMeta = [
    ...
    isDriver
      ? text(cargo?.company_name, deal?.shipper_company)
      : text(trip?.vehicle_brand, deal?.vehicle_brand),
    isDriver
      ? text(cargo?.country, deal?.shipper_country)
      : text(deal?.plate, trip?.vehicle_plate),
  ]
    .filter(Boolean)
    .join(" · ");
  ...
  const showLiveMap =
    !TERMINAL_STATUSES.includes(visibleDealStatus) &&
    visibleDealStatus !== "delivered" &&
  ...
  const inactiveSubtitle =
    visibleDealStatus === "delivered" ? ui.tripAwaitingReceipt : "";
  const inactiveHint =
    visibleDealStatus === "delivered" ? ui.tripAwaitingReceiptHint : "";

  const cancelDeal = async () => {
    const ok = await askConfirm(
      ui.cancelDeal,
      ui.cancelDealConfirm,
      ui.cancelDeal,
      true,
    );
    if (ok) {
      setStatusModalOpen(false);
      await changeDealStatus("cancelled");
    }
  };
      ...
      >
                  ...
                  <View style={s.recordBar} testID="deal-chat-recording-bar">
                    ...
                    <View style={s.recordWave} pointerEvents="none">
                      {[0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
                        <View
                          key={i}
                          style={[s.recordWaveBar, { height: 5 + (i % 4) * 4 }]}
                        />
                      ))}
                    </View>
                    ...
                    <TouchableOpacity
                      ...
                      testID="deal-chat-recording-cancel"
          ...
          <View style={s.mapFullscreen} testID="deal-map-fullscreen">
            <View style={s.mapArea} testID="deal-map-first-area">
              {showLiveMap ? (
                ...
                <View
                  style={[s.finishedMap, { backgroundColor: colors.surface }]}
                  testID="deal-inactive-map-summary"
                >
                  ...
                  <Text style={[s.finishedTitle, { color: colors.text }]}>
                    {inactiveTitle}
                  </Text>
                  <Text style={[s.finishedRoute, { color: colors.text }]}>
                    {routeLabel}
                  </Text>
                  {inactiveSubtitle ? (
                    <Text style={[s.finishedSubtitle, { color: colors.text }]}>
                      {inactiveSubtitle}
                    </Text>
                  ) : null}
                  {inactiveHint ? (
                    <Text style={[s.finishedHint, { color: colors.textMuted }]}>
                      {inactiveHint}
                    </Text>
                  ) : null}
                  <Text
                    style={[s.finishedGpsHint, { color: colors.textMuted }]}
                  >
                    {ui.mapFinishedHint}
                  </Text>
                </View>
              ...
              <TouchableOpacity
                ...
                onPress={closeMap}
                testID="deal-map-collapse"
              >
                <Feather name="minimize-2" size={17} color={colors.text} />
                <Text style={[s.mapCollapseText, { color: colors.text }]}>
                  {ui.collapseMap}
                </Text>
              </TouchableOpacity>
              ...
              {showLiveMap && updatedText ? (
                ...
                >
                  ...
                  <Text style={[s.updatedText, { color: colors.text }]}>
                    {updatedText}
                  </Text>
                </View>
                ...
                >
                  <View style={s.metricCell}>
                    <Text style={[s.metricLabel, { color: colors.textMuted }]}>
                      {routeSummary.isRemaining ? ui.remaining : ui.distance}
                    </Text>
                    <Text
                      style={[s.metricValue, { color: colors.text }]}
                      numberOfLines={1}
                    >
                  ...
                  <View style={s.metricCell}>
                    <Text style={[s.metricLabel, { color: colors.textMuted }]}>
                      {routeSummary.isRemaining ? ui.eta : ui.travelTime}
                    </Text>
                    <Text
                      style={[s.metricValue, { color: colors.text }]}
                      numberOfLines={1}
            ...
            <TouchableOpacity
              ...
              onPress={closeMap}
              testID="deal-chat-dock"
            >
              <View style={[s.chatIconBox]}>
                <Feather name="message-circle" size={18} color="#168759" />
              </View>
              <View style={{ flex: 1, minWidth: 0 }}>
                <View style={s.sheetTitleRow}>
                  ...
                  {unreadCount > 0 ? (
                    <Text style={s.newCount}>{unreadCount}</Text>
                  ) : null}
                </View>
                <Text
                  style={[s.preview, { color: colors.textMuted }]}
                  numberOfLines={1}
                >
                  {latestPreview}
                </Text>
        ...
        >
          <Pressable
            ...
            testID="deal-chat-photo-fullscreen"
          >
            {fullImage ? (
              <Image
        ...
        >
          ...
          >
            <Pressable
              style={[
                s.statusModalCard,
                { backgroundColor: colors.bg, maxHeight: window.height * 0.75 },
              ]}
              onPress={() => {}}
              testID="deal-status-panel"
            >
...
const s = StyleSheet.create({
  ...
  systemText: {
    ...
    paddingHorizontal: 10,
    paddingVertical: 5,
    backgroundColor: "rgba(124,139,130,0.12)",
    borderRadius: 999,
  },
  photoShell: { position: "relative" },
  ...
  transcriptBlockThem: { borderTopColor: "#E5ECE8" },
  transcriptLabel: {
    fontSize: 10,
    fontWeight: "800",
  ...
  transcriptText: { fontSize: 12, lineHeight: 17 },
