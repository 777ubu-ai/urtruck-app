...
## AI skills: краткие отчёты и пользовательский текст

В репозитории подключены локальные skills:

- `.claude/skills/concise/SKILL.md` — использовать по умолчанию для коротких
  owner-facing статусов, QA-сводок и промежуточных отчётов. Экономить слова,
  но не доказательства: сохранять точные PASS/FAIL, SHA, версии, устройства,
  команды, ошибки, риски и release blockers.
- `.claude/skills/humanizer/SKILL.md` — использовать только для
  пользовательского/маркетингового текста, описаний, писем и контента, когда
  нужен естественный язык. Не применять к коду, SQL, JSON, API-контрактам,
  логам, тестовым результатам и security/release evidence.

Для security, необратимых действий, релизных инструкций и сложного debugging
краткость не должна ухудшать точность или скрывать риски. Если владелец просит
подробности, дать полный разбор, затем вернуться к краткому режиму.
