...
set_env() {
  ...
  # Remove all old entries for this key, preserve every other line, append one value.
...
# Independent signing/QA keys are generated once on the server and never leave it.
