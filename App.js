...
function AppInner() {
  ...
  // P2 2026-09-03: канонический Android notification channel создаём на старте
  // БЕЗУСЛОВНО — до логина, без токена и без выданного POST_NOTIFICATIONS.
  //
  // Физический QA показал в NotificationRecord `fcm_fallback_notification_channel`
  // вместо `urtruck_messages_v2`: Firebase SDK создаёт этот служебный fallback,
  // если указанного в сообщении канала на устройстве ещё нет. Канал создавался
  // только внутри push.autoRegister() → registerNative(), а тот effect ниже
  // требует hasToken — то есть после свежей установки первый пуш мог прийти
  // раньше, чем канал вообще появился. Создание канала идемпотентно и
  // разрешения не требует, поэтому безопасно делать это отдельно и всегда.
  useEffect(() => {
    push.ensureAndroidChannel?.().catch(() => {});
  }, []);
