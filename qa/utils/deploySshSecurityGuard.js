...
const triggerWorkflow = fs.readFileSync('.github/workflows/secure-production-deploy.yml', 'utf8');
const deployWorkflow = fs.readFileSync('.github/workflows/production-deploy-execute.yml', 'utf8');
...
assert.ok(/workflow_run:/.test(triggerWorkflow), 'secure deploy must run after legacy deploy completion');
assert.ok(/environment: production/.test(deployWorkflow), 'production deploy must target production environment');
assert.ok(/permissions:\s*\n\s*contents: read/.test(deployWorkflow), 'GitHub token must be read-only');
assert.ok(/cancel-in-progress: false/.test(deployWorkflow), 'production deploy must never be cancelled mid-flight');
assert.ok(!/sshpass\s+-p/.test(deployWorkflow), 'secure deploy workflow must not invoke password SSH directly');
assert.ok(!/StrictHostKeyChecking=no/.test(deployWorkflow), 'secure deploy workflow must not disable host verification directly');
assert.ok((deployWorkflow.match(/scripts\/deploy-ssh\.sh/g) || []).length >= 8, 'all server transport must go through deploy-ssh helper');
