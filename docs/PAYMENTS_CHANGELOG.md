# UrTruck Pro Test — отдельная платёжная тестовая установка

Текущая ветка отделена от боевого приложения:

- имя: `UrTruck Pro Test`;
- Android package / iOS bundle ID: `com.urtruck.protest`;
- backend: отдельный каталог, база и порт `8002`;
- Google Play: только отдельная карточка и Internal Testing;
- лимиты: Free — 5 принятых сделок, Pro — 30.

Ни один workflow этой ветки не должен загружать AAB в карточку
`com.urtruck.app`.

## История исходной платёжной реализации
