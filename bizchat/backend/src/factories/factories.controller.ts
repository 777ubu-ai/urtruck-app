...
export class FactoriesController {
  ...
  async list(
    ...
    // Прячем «черновые» профили: при регистрации заводу присваивается
    // техническое имя вида «Factory 1e385061» (кусок идентификатора), пока
    // владелец не заполнил карточку. В витрине такие записи выглядят как
    // мусор, поэтому показываем только заводы с осмысленным названием.
    // Данные не удаляются — профиль появится в каталоге сразу после
    // переименования компании.
    qb.andWhere("f.companyName !~ '^Factory [0-9a-f]{8}$'");
