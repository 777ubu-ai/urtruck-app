...
function resetMarker(){ return join(process.cwd(), '.admin-data', 'owner-reset.used'); }
function markResetUsed(){ const p=resetMarker(); mkdirSync(dirname(p),{recursive:true,mode:0o700}); writeFileSync(p,new Date().toISOString(),{mode:0o600}); }
...
export async function POST(request: NextRequest) {
  ...
  const resetEnabled = String(process.env.ADMIN_OWNER_RESET_ENABLED || '').toLowerCase() === 'true';
